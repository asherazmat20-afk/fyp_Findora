...
function MyLostItems() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        const myLost = Array.isArray(data)
          ? data.filter(
              (item) =>
        ...
        console.error(error);
        setToast("Failed to load lost items");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <UiLoader text="Loading your lost items..." />;

  return (
    <PageTransition>
      <div className="min-h-screen bg-slate-50 p-6">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl font-bold mb-5 text-blue-700">My Lost Items</h2>
